...
//Output-education
const OutputEducation = React.createClass({
    renderEducation: function(){
        var educations = [];
        this.props.educationList.forEach(function(education){
            educations.push(
                <div className="panel">
                    <div className="panel-heading">
                        <h4><strong>{education.name}</strong><span className="label label-default">
                            {education.startedMonth}.{education.startedYear}-{education.finishedMonth}.{education.finishedYear}</span></h4>
                    </div>
    ...
    render: function(){
        if (typeof this.props.educationList !== "string"){
            return (
                <div className="panel">
                    <div className="panel-heading"><h3>Education</h3></div>
                    <div className="panel-body output-panel-body">{this.renderEducation()}</div>
                </div>
            );
        }else {
           return (
                <div className="panel">
                    <div className="panel-heading"><h3>Education</h3></div>
                    <div className="panel-body output-panel-body"><p>{this.props.educationList}</p></div>
                </div>
            ); 
        }
    }
...
//Input-work
var InputWork = React.createClass({
    ...
    initializeYear: function(){
        ...
        for(var i=currentYear;i>currentYear-50;i--){
            years.push(<li role='presentation'><a role='menuitem' href='#' onClick={this.onClickTime}>{i}</a></li>);
        }
        return years;
    },
    initializeMonth: function(){
        ...
        monthName.forEach(function(month){
            months.push(<li role="presentation"><a role="menuitem" href="#" onClick={this.onClickTime}>{month}</a></li>);
        }.bind(this));
        return months;
    },
